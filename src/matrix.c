...
double Cofactor(double *matrix,const int dimension,const double element,const int position_i,const int position_j)
{
    ...
    cofactor = pow(-1, (position_i+position_j) ) * Determinant(auxiliar_matrix,dimension-1);
    // o cofator de a(ij) eh dado por : -1^(i+j) * D(ij), em que D(ij) é o determinant e da matriz formada pelos
...
double Determinant(double *matrix,const int dimension)
{
    ...
    if( dimension == 2)
        ...
        for( j = 0; j < dimension; j++)
            ...
            determinant += *(matrix+j) * Cofactor(matrix,dimension,*(matrix+j),1,j+1);
            // o determinant eh dados pelo somatorio
